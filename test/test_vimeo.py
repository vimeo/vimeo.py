...
for key in password.dev_data:
    if key not in prod_data:
        prod_data[key] = password.dev_data[key]
...
def setup(dev):
    global vimeo
    global test_data
    test_data = password.dev_data if dev else prod_data
...
class VimeoObjectTest():
    def search(self,
        ...
        with pytest.raises(Exception):
            r = getattr(vimeo, resource)(_filter="test", msg="%s \
            search should fail without query params")
    ...
    def single_lookup(self,
        ...
        r = resource()
        assert r['body']['name'], "Single %s lookup should return \
        a resource" % (str(resource))

        r = resource(_filter='test')
        assert r['body']['name'], "Single %s lookup with filter \
        should return a resource" % (str(resource))

        r = resource(content_filter='safe')
        assert r['body']['name'], "Single %s lookup with content \
        filter should return a resource" % (str(resource))

        def handle(response, error):
            assert response['body']['name'], "Single %s lookup should \
            work asynchronously" % (str(resource))
    ...
    def multi_lookup(self,
        ...
        assert r['body'], "%s %s lookup should return an object\
        " % (str(resource), endpoint)
        assert 'data' in r['body'], "%s %s lookup should return \
        some data" % (str(resource), endpoint)

        r = getattr(resource, endpoint)(query='test')
        assert r['body'], "%s %s lookup with search should return \
        an object" % (str(resource), endpoint)
        assert 'data' in r['body'], "%s %s lookup with search should \
        return some data" % (str(resource), endpoint)

        r = getattr(resource, endpoint)(_filter='test')
        assert r['body'], "%s %s lookup with filter should return an \
        object" % (str(resource), endpoint)
        assert 'data' in r['body'], "%s %s lookup with filter should \
        return some data" % (str(resource), endpoint)

        r = getattr(resource, endpoint)(content_filter='safe')
        assert r['body'], "%s %s lookup with content filter should \
        return an object" % (str(resource), endpoint)
        assert 'data' in r['body'], "%s %s lookup with content filter \
        should return some data" % (str(resource), endpoint)

        r = getattr(resource, endpoint)(sort='date')
        assert r['body'], "%s %s lookup with sort should return an \
        object" % (str(resource), endpoint)
        assert 'data' in r['body'], "%s %s lookup with sort should \
        return some data" % (str(resource), endpoint)

        def handle(response, error):
            assert 'data' in response['body'], "%s %s lookup should \
            work asynchronously" % (str(resource), endpoint)
    ...
    def exists(self,
        ...
        assert True, "%s GET should show exising %s\
        " % (str(resource), endpoint)
    ...
    def get_item(self,
        ...
        assert put_id in r['body']['uri'], "%s should \
        be able to get %s" % (str(resource), endpoint)
    ...
    def post_item(self,
        ...
        for item in r['body']['data']:
            if data[key_field] == item[key_field]:
                added_id = item['uri']
                break
        assert added_id, "%s %s should be able to post a \
        %s" % (str(resource), identifier, endpoint)
        return added_id
    ...
    def patch_item(self,
        ...
        if 'data' in r['body']:
            for item in r['body']['data']:
                if user:
                    if data[key_field] == item['user'][response_field]\
                       .split('/')[-1]:
                        added_id = item['uri']
                        break
                else:
            ...
            if user:
                if r['body']['user'][response_field].split('/')[-1]\
                   == data[key_field]:
                    added_id = r['uri']
            else:
                if r['body'][response_field] == data[key_field]:
                    added_id = r['uri']
        assert added_id, "%s %s should be able to patch a %s\
        " % (str(resource), identifier, endpoint)
...
class TestClient():
    def test_init(self):
        ...
        assert vimeo.config['access_token'], "VimeoClient should have an \
        access token"
    ...
    def test_auth_modes(self, dev):
        vimeo = VimeoClient(access_token=test_data['access_token'],
                            dev=bool(dev))
        r = vimeo.me.videos()
        assert 'data' in r['body'], "Access token auth should work"

        #vimeo = VimeoClient(client_id=test_data['client_id'],
...
class TestUser(VimeoObjectTest):
    ...
    def test_me(self):
        ...
        assert me['body']['uri'] == user['body']['uri'], "/me should \
        return an identical uri"
    ...
    def test_user_following(self):
        ...
        self.put_item('users', test_data['user'], 'following',
                      test_data['followuser'])
        self.delete_item('users', test_data['user'], 'following',
                         test_data['followuser'])
    ...
    def test_user_likes(self):
        ...
        self.put_item('users', test_data['user'], 'likes',
                      test_data['likevideo'])
        self.delete_item('users', test_data['user'], 'likes',
                         test_data['likevideo'])
    ...
    def test_user_portfolios(self, dev):
        ...
        assert 'data' in r['body'], "User portfolio lookup should \
        return some data"

        def handle(response, error):
            assert 'data' in response['body'], "User portfolio lookup \
            should work asynchronously"
        user.portfolios(_callback=handle)

        r = user.portfolios.get(test_data['portfolio']).videos()
        assert r['body'], "User portfolio videos lookup should return \
        an object"
        assert 'data' in r['body'], "User portfolio videos lookup should \
        return some data"
        ...
        r = user.portfolios.get(test_data['portfolio']).videos.get("70429\
        873")()
        assert r['body'], "User portfolio video lookup should return an object"
        assert 'uri' in r['body'], "User portfolio video lookup should \
        return a video"

    def test_user_presets(self):
        self.multi_lookup('users', test_data['user'], 'presets')
        self.exists('users', test_data['user'], 'presets',
                    test_data['presets'])
    ...
    def test_user_videos(self):
        ...
        assert r is not None, "User single video GET should succeed for \
        owned video"
        with pytest.raises(Exception):
    ...
    def test_user_watchlater(self):
        ...
        self.put_item('users', test_data['user'], 'watchlater',
                      test_data['likevideo'])
        self.delete_item('users', test_data['user'], 'watchlater',
                         test_data['likevideo'])
...
class TestVideo(VimeoObjectTest):
    ...
    def test_videos_single(self):
        ...
        # reset for next run
        vimeo.videos.get(test_data['editvideo']).patch({'name': 'Nellie \
        Krumping'})

    def test_video_comments(self):
        self.multi_lookup('videos', test_data['editvideo'], 'comments')

        pid = self.post_item('videos', test_data['editvideo'], 'comments',
                             {'text': 'testing text'})
        ...
        assert '/'.join(pid.split('/')[-2:]) in r['body']['uri'], "%s should be \
        able to get comments" % (str(resource))

        self.patch_item('videos', test_data['editvideo'], 'comments',
                        pid, {'text': 'edited text'})
    ...
    def test_video_credits(self):
        ...
        pid = self.post_item('videos', test_data['editvideo'], 'credits',
                             {"name": 'tester', "role": "testee"},
                             key_field='name')
        self.get_item('videos', test_data['editvideo'], 'credits', pid)
        self.patch_item('videos', test_data['editvideo'], 'credits', pid,
                        {'role': 'edited text'})
    ...
    def test_video_presets(self):
        self.put_item('videos', test_data['editvideo'], 'presets',
                      test_data['presets'], check_get=False)
        self.exists('videos', test_data['editvideo'], 'presets',
                    test_data['presets'])
        self.delete_item('videos', test_data['editvideo'], 'presets',
                         test_data['presets'], check_get=False)
    ...
    def test_video_tags(self):
        ...
        vimeo.videos.get(test_data['editvideo']).tags.putmulti(['test\
        one', 'testtwo'])
...
class TestChannel(VimeoObjectTest):
    ...
    def test_channel_create_delete(self):
        r = vimeo.channels.post({'name': 'testchannel',
        ...
        for channel in r['body']['data']:
            assert cid not in channel['link'], "Channels should be able \
            to delete channels"
    ...
    def test_channels_single(self):
        ...
        # reset for next run
        vimeo.channels.get(test_data['channel']) .patch({'name': 'something\
        else'})
...
class TestGroup(VimeoObjectTest):
    ...
    def test_group_create_delete(self):
        ...
        for group in r['body']['data']:
            assert gid not in group['link'], "Groups should be able to delete \
            groups"
...
class TestUpload(VimeoObjectTest):
    def test_upload(self):
        ...
        def hook(_range):
            assert _range > 0, "Uploading with a post-check hook should \
            call the hook"
        try:
            uid = vimeo.upload(test_data['uploadvideo'], post_check_hook=hook)
        except IOError:
            raise IOError("You must create a file in your cwd called \
            testupload.mp4 in order to test uploads")
        total = vimeo.me.videos()['body']['total']
        assert total - 1 == old_total, "Uploading a video should add 1 to \
        user video count"
