...
the programmer can use the library to guide a user through the authorization
flow, allowing that user to create their own access token.

The functions for this flow can be found in vimeo/auth.py
"""
if __name__ == "__main__":
    args = None
    parser = argparse.ArgumentParser(description='Generate an API access \
                                     token')
    parser.add_argument('--cid', '-i', help="Your client ID", nargs=1,
                        required=True)
    parser.add_argument('--secret', '-s', help="Your client secret",
                        nargs=1, required=True)
    parser.add_argument('--redirect', '-r', help="Your app's redirect URI",
                        nargs=1, required=True)
    ...
    def do_auth_flow(api_root, cid, secret, scopes, redirect):
        print "Visit %s in a browser" % get_auth_url(api_root, cid, scopes,
                                                     redirect)
